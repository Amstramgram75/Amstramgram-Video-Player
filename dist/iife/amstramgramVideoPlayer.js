...
var AmstramgramVideoPlayer = (function () {
  ...
  */
  ...
  var _isIosDevice = typeof window !== 'undefined' && window.navigator && window.navigator.platform && /iP(ad|hone|od)/.test(window.navigator.platform); //Détection de la fonctionnalité fullscreen et de ses éventuels préfixes
  ...
  function () {
    ...
    function AmstramgramVideoPlayer(el, params) {
      ...
      */
      ...
      wrapper.innerHTML = buildUI(this.params); //Insertion du wrapper dans le DOM
      ...
      function _play() {
        ...
        layerPlay.classList.add('amst__hidden'); //On programme la disparition de la barre de controle

        if (!pointerOverControls) _hideControls({
          detail: {
            delayed: true
          }
        });
      ...
      function _pause() {
        ...
        layerPlay.classList.remove('amst__hidden'); //On affiche la barre de contrôle.
      ...
      function _reset() {
        ...
        */
        ...
        layerPosterCanvas.getContext('2d').drawImage(media, 0, 0, layerPosterCanvas.width, layerPosterCanvas.height); //On affiche le layer-poster. Le poster qui constitue le background est caché par le canvas.

        layerPoster.classList.remove('amst__hidden'); //On reset le lecteur
      ...
      */
      ...
      function updateAllButtons() {
        //Fonction appelée lors de la mise à jour de la source
        _previous();
      ...
      function _src(e) {
        ...
        if (self.params.thumbnails.src && !_isIosDevice) {
          ...
          var thumbEvent = function thumbEvent(e) {
            ...
            if (e.type == 'load') {
              ...
              container.classList.add('amst__thumbnails'); //On détermine la largeur des vignettes
        ...
        if (self.params.playsinline === true) {
          attributes.playsinline = '', attributes['webkit-playsinline'] = '';
        }
        ...
        media.setAttributes(attributes); //Initialisation/Mise à jour du volume
      ...
      media.on('mousemove', function () {
        ...
        var myFormat = _this3.params.format = media.videoWidth / media.videoHeight; //Actualisation de la taille du container en fonction du format réel de la vidéo
        ...
        container.classList.add('amst__loadedmetadata'); //Si le lecteur a été reseté du fait du lancement d'un autre lecteur
        ...
        _this3.params.duration = media.duration; //Mise à jour de skipTime

        skipTime = typeof _this3.params.skipTime == 'string' && _this3.params.skipTime.slice(-1) == '%' ? parseFloat(_this3.params.skipTime) * media.duration / 100 : parseFloat(_this3.params.skipTime); //Mise à jour des champs indicateurs de temps
        ...
        $('.amst__currenttime').innerHTML = secondsToTimeCode(floorCurrentTime, media.duration > 3600); //On reprend une mesure des contrôles au cas où le container ait gagné ou perdu la class amst__long 
      ...
      .on('timeupdate', function () {
        ...
        if (floorCurrentTime == 0 && _this3.paused) {
      ...
      */
      ...
      $$([media, layerLoading, layerSeekingTouch, layerPlay]).on('touchstart', function (e) {
        ...
        distX = 0,
            //Variable destinée à stocker la distance parcourue sur l'axe X
        distY = 0,
            //Variable destinée à stocker la distance parcourue sur l'axe Y
        ...
        horizontalMove = 0,
            seekingRatio = undefined,
            //Variable définie uniquement dans le cas d'un déplacement horizontal
        ...
        function touchEnd(e) {
          ...
          if (seekingRatio != undefined) {
            ...
            media.currentTime = media.duration * seekingRatio; //On cache le layer seeking
      ...
      */
      ...
      rail.on(myPointerDown, function () {
        ...
        rail.on(myPointerMove, moveHandle, passive ? {
          passive: true
        } : false);
      ...
      function cleanTimeSliderMoveEvents(e) {
        ...
        if (!self.paused && !pointerOverControls) _hideControls({
          detail: {
            delayed: true
          }
        }); //On opère le déplacement de la tête de lecture ce qui provoque celui du curseur

        moveHandle(e); //On retire les écouteurs.
